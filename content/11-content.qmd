...
The formative model is expressed mathematically as follows:
...
- The **reflective model** statistically implies that the observed variables (indicators) are reflections or manifestations of the latent variable, expressed as $X_i = \lambda_i \eta + \varepsilon_i$. Structurally, this model makes the assumption that the latent variable is causally efficacious and influences the observed variables. However, as we shall see, these causal assumptions are strong and may not always hold true.

- The **formative model** statistically implies that the latent variable is formed or influenced by the observed variables, expressed as $\eta = \sum_i\lambda_i X_i + \varepsilon$. Structurally, this model makes the assumption that the observed variables give rise to the latent variable. As with the reflective model, the causal assumptions inherent in the formative model are strong and may not always hold true.
...
Statistically, the formative model implies $X_i = \lambda_i \eta + \varepsilon_i$. However, in the tradition of factor analysis, researchers have interpreted the formative model to imply a single univariate latent reality as implied by @fig-structural-assumptions-reflective-model. Such an interpretation has makes strong causal assumptions. 

Similarly, statistically, the formative model implies $\eta = \sum_i\lambda_i X_i + \varepsilon$. However, in the tradition of factor analysis, researchers have interpreted the formative model to imply a single underlying univariate latent reality as in  @fig-dag-reflective-assumptions_note. This too makes strong causal assumptions. Again, **cross sectional data cannot decide between these assumptions.**
...
In a similar vein, the statistical model $\eta = \sum_i\lambda_i X_i + \varepsilon$ concurs with @fig-dag-reflective-assumptions_note but also coheres with @fig-dag-reflectiveassumptions-compatible_again. Here too, cross-sectional data prove insufficient to distinguish between these potential structural interpretations.

The takeaway is that the data we collect can align with numerous possible realities, and we must be careful not to over-interpret the results from factor analysis.
...
We define a causal effect as the difference between expected potential outcomes under two levels of a treatment. In this context, each level of treatment is hypothetical, and it's understood that every individual is exposed to it, regardless of the reality of their exposure. This consideration is done under the condition of their levels of a confounder. 

Formally, this difference can be represented mathematically as follows:

$$ \delta = \sum_c \left( \mathbb{E}[Y|A=a,c] - \mathbb{E}[Y|A=a^*,c] \right) P(c)$$

Here, $\delta$ represents the causal estimand on the difference scale, i.e., $(\mathbb{E}[Y^0 - Y^0])$.

When dealing with causal inference, situations often arise where the treatment isn't uniform, but rather, has several variations. This is where the theory of multiple versions of treatments becomes significant. It allows us to model these variations and their potential differing impacts on the outcome.

In this framework, the consistency assumption is tailored to fit the context of multiple treatment versions. It dictates that the observed outcome under a specific version of the treatment is equivalent to the outcome that would have occurred had that version been administered.

Moreover, this theory requires a "no confounding" assumption, meaning that the potential outcomes for each treatment version are independent of the version received once the covariates are taken into account.

This absence of confounding for the effect of $K$ on $Y$ given $C$ can be symbolized as $Y_k \coprod K|C$, suggesting that $A$ offers no additional information about $Y$ once $K$ and $C$ are considered. The consistency assumption is satisfied if $Y = Y_k$ for $K = k$.

Suppose we only have data on $(A, Y, C)$. If $Y_k$ is independent of $K$ conditional on $C$, then there is no confounding of the effect of $K$ on $Y$ given $C$. This means that when $K = k$, $Y = Y_k$ holds true, satisfying the consistency assumption.

The causal effect in this context can then be represented as:

$$ \delta = \sum_{k,c} \mathbb{E}[Y_k|c] P(k|a,c) P(c) - \sum_{k,c} \mathbb{E}[Y_k|c] P(k|a^*,c) P(c)$$

This represents a hypothetical randomized trial. Under this scenario, within each stratum of the covariates $C$, one group of individuals is randomly assigned a treatment version $K$ from the inherent $K$ distribution in the subpopulation with $(A = a, C = c)$. Meanwhile, another group receives a version $K$ randomly drawn from the same distribution.

This theory proves especially beneficial in practical scenarios where treatment versions differ significantly -- which would be typical in observational studies. .

VanderWeele suggests that the theory of causal inference under multiple treatments could provide valuable tools for handling the consistency assumption, given these complexities.



## Theory of multiple versions of treatment applied to psychometric research

The theory of multiple versions of treatments holds that different variations of a treatment can have varying impacts on an outcome. In this framework, the consistency assumption is revisited, with the stipulation that the observed outcome corresponds to the outcome under the specific version of the treatment that was received. Importantly, this theory also necessitates the absence of confounding, given measured covariates for each version of the treatment.

VanderWeele builds on this theory, proposing a substitution:

$$\delta = \sum_{\eta,c} \mathbb{E}[Y_\eta|c] P(\eta|A=a+1,c) P(c) - \sum_{\eta,c} \mathbb{E}[Y_\eta|c] P(\eta|A=a,c) P(c)$$

Here, VanderWeele substitutes $K$ with $\eta$ from the previous section, comparing measurement level $A = a + 1$ with $A = a$. If the influence of $\eta$ on $Y$ is not confounded given $C$, $\Delta_{ate}$ maintains its interpretability as a comparison in a hypothetical randomized trial, even with varying reliabilities $\eta_i$.

Individuals within the covariate strata $C$ are randomly assigned an $\eta$ value, derived from the actual $\eta$ distribution in the subpopulation with $(A = a + 1, C = c)$ or $(A = a, C = c)$, depending on the group. This creates a strong link between measurement and the multiple versions of treatment framework, offering a robust tool for causal inference when measurement reliabilities vary.

However, similar to the theory of multiple treatments, interpretation of results may still pose challenges. 

The same considerations are applicable to the "formative model of measurement", which posits that the latent variable $\eta$ is causally efficacious. Yet again, interpreting results for this model, like with the reflective model and the theory of multiple treatments, might pose challenges due to potential unknown factors.
...
Consider a study investigating the impact of a person's true state of regular exercise, represented as $\eta_A$ in the diagram, on their true distress state, represented as $\eta_Y$. There is a potential confounder in this study, which is the person's true state of daily stress level, represented as $\eta_L$.
...
It's also assumed that these measurement errors, or unmeasured sources of error, affect the measured states: $U_{L} \rightarrow$ $L_{f(X_1\dots X_n)}$, $U_{A} \rightarrow$ $A_{f(X_1\dots X_n)}$, and $U_{Y} \rightarrow$ $Y_{f(X_1\dots X_n)}$.

Further, it's hypothesised that the errors are correlated such that a person experiencing high levels of stress ($U_{L}$) might inaccurately report their exercise levels and mood state, leading to an assumed correlation in errors: $U_{L} \rightarrow U_{A}$, and $U_{L} \rightarrow U_{Y}$.

Finally, the diagram assumes that the true state of exercise impacts the measurement error in mood, and the true state of stress impacts the measurement error in exercise. This is represented by the red arrows from $\eta_A \rightarrow U_{Y}$ and from $\eta_L \rightarrow U_{A}$. The connections from $U_{L}$ to $\eta_L$ and from $U_{A}$ to $\eta_A$ indicate the relationships between the unmeasured sources of error and the true states of stress and exercise, respectively.
...
In all of these processes, the measurements of stress, exercise, and distress states reflect a more complex underlying reality, and they are also subject to multiple unmeasured sources of error. Understanding this complexity and the dynamics is critical. 
...
**Key takehome message** we must therefore always perform sensitivity analyses. 
