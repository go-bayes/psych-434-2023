...
Should you choose to execute a model without applying propensity score weights, it would equate to performing ordinary g-computation. In this scenario, the regression model could be specified with `weights = 1` or simply left unspecified, which R interprets as `weights = 1` by default. 
...
> Thus, while both the categorical and continuous exposure results suggest a potential influence of exercise in reducing anxiety for NZ Europeans, and possibly for Māori, the effects seem to be less pronounced when accounting for continuous exercise exposure. It's important to note that each model probes a different question. The categorical exposure model investigates the contrast between not exercising at all and exercising over eight hours per week, whereas the continuous exposure model examines the contrast between exercising almost six hours a week and doubling that amount of exercise.

> These represent different hypothetical scenarios, and arguably, neither might reflect realistic circumstances. Nonetheless, the consistent indication of exercise potentially aiding in anxiety reduction tentatively suggests that it would be worthwhile to further investigate the role of exercise in promoting mental health...
